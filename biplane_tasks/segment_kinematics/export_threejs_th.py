"""Export thoracohumeral kinematics for three.js animations for every trial in the biplane/Vicon
...
if __name__ == '__main__':
    ...
    # initialize
    config_dir = Path(mod_arg_parser('Export thoracohumeral kinematics for three.js', __package__, __file__))
    ...
    def trial_exporter(trial, subject_folder):
        torso_fluoro = change_cs(trial.subject.f_t_v, trial.torso_vicon[trial.vicon_endpts[0]:trial.vicon_endpts[1]])
        th = change_cs(ht_inv(torso_fluoro[trial.humerus_biplane_data.index.to_numpy() - 1]), trial.humerus_fluoro)
        pos = th[:, :3, 3]
        quat = Rot.from_matrix(th[:, :3, :3]).as_quat()
